...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.amountLimit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.amount-limits.update", [$amountLimit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="royalties">{{ trans('cruds.amountLimit.fields.royalties') }}</label>
                <input class="form-control {{ $errors->has('royalties') ? 'is-invalid' : '' }}" type="number" name="royalties" id="royalties" value="{{ old('royalties', $amountLimit->royalties) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('advance_limit') ? 'is-invalid' : '' }}" type="number" name="advance_limit" id="advance_limit" value="{{ old('advance_limit', $amountLimit->advance_limit) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('artist') ? 'is-invalid' : '' }}" name="artist_id" id="artist_id" required>
                    ...
                    <option value="{{ $id }}" {{ (old('artist_id') ? old('artist_id') : $amountLimit->artist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
